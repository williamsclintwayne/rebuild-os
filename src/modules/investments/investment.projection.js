export function projectInvestment(config) {
    const { monthlyContribution, annualIncrease, annualReturn, years, } = config;
    ...
    for (let year = 0; year < years; year++) {
        const adjustedMonthly = monthlyContribution *
            Math.pow(1 + annualIncrease / 100, year);
        const yearlyContribution = adjustedMonthly * 12;
        let tfsaContribution = Math.min(yearlyContribution, annualTfsaLimit);
        ...
        const taxableContribution = yearlyContribution - tfsaContribution;
        const totalContribution = tfsaContribution + taxableContribution;
        portfolio =
        ...
        if (tfsaLifetimeUsed >= lifetimeTfsaLimit) {
            for (let remaining = year + 1; remaining < years; remaining++) {
                const nextMonthly = monthlyContribution *
