// debt.projection.ts
export function simulateSnowball(debts, config) {
...
export function projectDebtYearly(debts, extraPayment) {
    const cloned = debts.map(d => ({ ...d }));
    const yearlyData = [];
    let months = 0;
    while (cloned.some(d => d.currentBalance > 0) && months < 600) {
        ...
        if (months % 12 === 0) {
            const totalRemaining = cloned.reduce((sum, d) => sum + d.currentBalance, 0);
            yearlyData.push(totalRemaining);
        }
    }
    return yearlyData;
}
export function estimateDebtPayoffMonths(debt) {
...
export function generateAmortization(debt) {
    const schedule = [];
    let balance = debt.currentBalance;
    let month = 0;
    const monthlyRate = debt.interestRate / 100 / 12;
...
export function compareInterest(debts, extraPayment) {
    ...
    return {
        ...
        interestSaved: minimum.totalInterestPaid - aggressive.totalInterestPaid,
    };
}
